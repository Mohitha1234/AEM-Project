...
public class PageExpiryScheduler implements Runnable {
    ...
    public void run() {
        ...
        try (ResourceResolver resolver = resolverFactory.getServiceResourceResolver(
            ...
            Resource contentRoot = resolver.getResource("/content/task01");
            if (contentRoot == null) {
            ...
            while (pages.hasNext()) {
                ...
                try {
                    ...
                    // Optional: publish if exactly now (±1 minute tolerance)
                    else if (Math.abs(expiryDate.getTimeInMillis() - now.getTimeInMillis()) < 60_000) {
                        replicator.replicate(session, ReplicationActionType.ACTIVATE, pagePath);
                        log.info(" Published (Current): {}", pagePath);
                    } else {
                        log.debug(" Page {} has future expiry date. No action taken.", pagePath);
                    }
